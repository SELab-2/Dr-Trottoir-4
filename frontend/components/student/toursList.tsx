                    ...
                    <div className="list-group">
                        {
                            studentOnTours.map(el => {
                                return (
                                    <a onClick={() => onSelect(el.id, allTours[el.tour] && allRegions[allTours[el.tour].region] ? allRegions[allTours[el.tour].region].id : 0)}
                                       className="list-group-item list-group-item-action" key={el.id}>
                                        <div className="d-flex w-100 justify-content-between">
                                            <h5 className="mb-1">{allTours[el.tour] ? allTours[el.tour].name : ""}</h5>
                                            <small>{el.date.toLocaleDateString('en-GB')}</small>
                                        </div>
                                        <p className="mb-1">{allTours[el.tour] && allRegions[allTours[el.tour].region] ? allRegions[allTours[el.tour].region].region : ""}</p>
                                    </a>
                                );
                            })
                        }
