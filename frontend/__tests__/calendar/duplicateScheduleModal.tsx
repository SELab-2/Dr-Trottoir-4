...
describe('DuplicateScheduleModal', () => {
    ...
    it('renders without crashing', () => {
        render(<DuplicateScheduleModal show={true} closeModal={mockCloseModal} title={'Test Title'}
                                       onSubmit={mockOnSubmit} weekStartsOn={0}/>);
    });

    it('handles form submission', async () => {
        const {getByLabelText, getByText} = render(<DuplicateScheduleModal show={true} closeModal={mockCloseModal}
                                                                           title={'Test Title'} onSubmit={mockOnSubmit}
                                                                           weekStartsOn={0}/>);
    ...
    it('closes the modal', () => {
        const {getByText} = render(<DuplicateScheduleModal show={true} closeModal={mockCloseModal} title={'Test Title'}
                                                           onSubmit={mockOnSubmit} weekStartsOn={0}/>);
